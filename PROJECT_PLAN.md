...
**Resolved, with evidence:** PrayerFly has never been deployed to any public `*.vercel.app` URL under this account — confirmed via the Vercel MCP connection (`list_projects` shows only one project on the account, "sarzameenz," an unrelated real-estate marketplace app; `list_deployments` on it shows 18 deployments, none of them PrayerFly). Local `vercel ls`/`vercel whoami` fail for a separate, expected reason — this machine's CLI was never logged in. So Sprint 12a's UI/accessibility work (tashkeel toggle, share-image, contrast ratios, mobile viewport) was legitimately verified against localhost and doesn't need repeating — none of it depended on public deployment.

**Update: now deployed, and currently fully public with no indexing protection — confirmed by direct fetch.** `https://prayerfly.vercel.app` is live. `robots.txt` returns `Allow: /` for all agents (with a `Sitemap:` line already pointing at the eventual `prayerfly.com` production URL), and the page `<head>` has no `noindex` meta tag. The Sprint 12a "indexing safety net" task was never actually implemented in code (consistent with the earlier finding that no such code existed anywhere in the repo) — it needs to be added now, against this real live URL, before anything else. Realistic near-term risk is low (fresh URL, no inbound links, nothing submitted to Search Console yet), but there's no reason to leave it exposed longer than the fix takes.
